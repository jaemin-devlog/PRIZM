...
|---|---|---:|---|---|
| `EVAL-R1-01` | 프로젝트 구조 및 코드 완성도 | 6 | `PARTIAL` — 인증·ownership·version·비동기 복구·검색과 실제 OpenSQL single-node SQL Gate가 있음 | clean-clone, demo `USER`, 처리 완료 확인, OpenSQL+Ollama 전체 흐름, browser E2E 또는 고정 수동 UI 시험표 |
| `EVAL-R1-02` | 오픈소스 프로젝트 발전 가능성 | 6 | `PARTIAL` — Engine·Reference App 방향, source-only Apache-2.0·NOTICE·SBOM·AI 모델 명세와 검증 CI·최종 감사가 있음 | 안전한 demo `USER`를 포함한 clean-clone Quickstart, 안정된 확장 경계; 외부 기여 운영 문서는 실제 접수·지원 release 전까지 `DEFERRED` |
| `EVAL-R1-03` | 개발 문서의 구체성 | 6 | `PARTIAL` — 현황·roadmap·spec·OpenSQL 실행 evidence·이 추적표가 실제 PR·CI·merge에 연결됨 | clean-clone으로 검증된 설치·운영·troubleshooting, architecture/data flow |
| `EVAL-R1-04` | 프로젝트 혁신성 | 6 | `PARTIAL` — 원문 출처 검색, version fencing, 안전한 복구가 있음 | 공식 과제의 동기화·MCP·가용성 증거와 PRIZM 차별 slice |
| `EVAL-R1-05` | 프로젝트 팀워크 | 6 | `PARTIAL` — 개인 참가자의 실제 spec→branch→PR→CI→merge 흐름이 반복됐고 review 부재도 정직하게 기록됨 | 신규 작업의 실제 Issue·review와 community 증거; 과거 Issue·review는 소급 생성하지 않음 |
...
| 항목 | 값 |
|---|---|
| Assessment ID | `ASSESS-2026-07-30-02` |
| Type | `INTERNAL_ESTIMATE_NOT_OFFICIAL` |
| Scope | `PRE_SUBMISSION_REPOSITORY_ONLY` |
| Official score | `N/A` |
| Assessment target commit | `777e184f206d2a2770d055940ddabf139abfed9d` |
...
|---|---:|---|
| `EVAL-R1-01` | 4/6 | OpenSQL+Ollama clean-clone에서 전체 사용자 흐름과 UI 시험 `PASS` |
| `EVAL-R1-02` | 3/6 | 안전한 demo `USER`를 포함한 clean-clone Quickstart 검증 완료; CONTRIBUTING·SECURITY는 실제 외부 운영 시작 전 `DEFERRED` |
...
|---|---|---|---|---|---|
| 2026-07-24 | `ASSESS-2026-07-24-01` | `2cb1bc49c4bfdf40c51a0adb347367e0f4602491` | 기준선 `N/A → 17/30` | source·test·문서·GitHub 이력에 대한 세 갈래 읽기 전용 평가 | `STALE` |
| 2026-07-30 | `ASSESS-2026-07-30-02` | `777e184f206d2a2770d055940ddabf139abfed9d` | `17/30 → 17/30`; 증거 보강, 점수 자동 상승 없음 | PRZ-002 source-only 최종 감사·[PR #22](https://github.com/jaemin-devlog/PRIZM/pull/22) merge `42876b6`, PRZ-003 실제 OpenSQL Gate·[PR #24](https://github.com/jaemin-devlog/PRIZM/pull/24) merge `777e184`·성공 CI | `CURRENT` |
...
| 라이선스 검증 | 5 | 오픈소스SW 역량프라자 의뢰 온라인 검증, 소스 업로드, 사용 OSS와 복수 라이선스 충돌 및 해결 방안 확인 | `PARTIAL` — source-only Apache-2.0 `LICENSE`·`NOTICE`, dependency·model·data·asset 감사, SBOM·AI 모델 명세와 검증 CI가 있음 | 외부 분석 결과, 외부 제공 OpenSQL runtime의 사용·비배포 경계와 충돌 해결표 |
...
| 오픈소스SW 적절성 | 10 | 다른 OSS를 적절히 도입·활용해 정상 운영되는 수준 | `PARTIAL` — 실제 OpenSQL single-node SQL Gate와 source-only OSS inventory가 있음 | OpenSQL+Ollama 전체 사용자 흐름, OSS 선택 이유·버전·용도·라이선스 표 |
| 기능테스트 | 10 | 외부 기능검증 결과 | `PARTIAL` | 외부 검증 가능한 실행 절차와 정상·예외·장애 test 결과 |
| 라이선스 검증 | 5 | 외부 라이선스 분석·식별·충돌 검증 결과 | `PARTIAL` — source-only 감사·LICENSE·NOTICE·SBOM·AI 모델 명세·검증 CI가 있음 | 외부 분석 결과와 외부 제공 OpenSQL runtime의 사용·비배포 경계를 포함한 충돌 여부·해결 방안 |
...
1. 공식 오리엔테이션 원본 URL·version·hash 고정과 source-only 라이선스 감사
   — `COMPLETE`; 외부 기여·보안 운영 문서는 재개 조건까지 `DEFERRED`
2. 실제 OpenSQL 단일 환경 Gate — `COMPLETE`
