...
`IN_PROGRESS` — S2C-03 기본 profile 승격과 S2C-04 전체 회귀 fixture 정렬을 포함한
제품 범위는 `main`에 통합됐다. 의미 단위 청킹·batch embedding·PDF 중복 최적화의
제품 적용 Gate는 남아 있다.

- 실행일: `2026-08-07`–`2026-08-08`
- 통합 source: `2190d47ff013384cf9d8c441449149233f79b0e9`
- 시작 기준: `83631f13c21eab54ac0f32ebb0f893b6c5acea0f`
...
- GitHub 통합: [PR #40](https://github.com/jaemin-devlog/PRIZM/pull/40), merge commit
  `9b24808b37424f2d11ca0afe374d5703c81868fc`
...
```text
고정 dataset과 검색 profile 선택
↓
owner·ACTIVE 후보 검색과 근거 판정
↓
세 결과 상태·snippet·원문 근거 응답
↓
PostgreSQL 평가와 OpenSQL direct 5432 제품 Gate를 별도 실행
↓
API·UI·전체 회귀 판정
```

PostgreSQL 평가 결과와 OpenSQL direct 제품 Gate는 서로 대체하지 않는다. 평가 전용
청킹·sparse·reranker 비교도 제품 적용 결과로 확대하지 않는다.

## 실행 결과

- **범위:** exact-token 대상 회귀
  - 명령: `.\gradlew.bat test --tests "com.prizm.search.evaluation.SearchEvaluationCompositeProfileTest" --tests "com.prizm.search.service.SearchServiceTest" --no-daemon --rerun-tasks`
  - 결과: `PASS` — Unicode 경계·단일 고유명사·완료 행위 양성/음성 포함
- **범위:** backend unit 전체
  - 명령: `.\gradlew.bat test --no-daemon --rerun-tasks`
  - 결과: `PASS` — 350개 중 335 pass, Windows 파일시스템 제약·OpenSQL 승인 Gate 15 skip
- **범위:** backend integration 전체
  - 명령: `.\gradlew.bat integrationTest --no-daemon --rerun-tasks`
  - 결과: `PASS` — 78개 중 75 pass, Windows 파일시스템 제약 2개·OpenSQL opt-in 1개 skip
- **범위:** 제품 source TUNING 15
  - 명령: `.\gradlew.bat searchEvaluation --no-daemon --rerun-tasks ...`와 `TUNING`·opt-in profile 고정
  - 결과: `PASS` — top-1 8/8·오타 2/2·중복 0·무관 거부 1.0·오거부 0
- **범위:** 고정 Dataset TEST
  - 명령: legacy·opt-in 각각 최종 실행
  - 결과: `PASS` — 모든 품질 Gate 충족
- **범위:** 실제 OpenSQL 제품 Gate
  - 명령: direct 5432 API·UI 실행
  - 결과: `PASS` — 세 상태·owner/ACTIVE/TXT/PDF·UI 근거 표시
- **범위:** P18 P8 40 + P17 28
  - 명령: Product와 `short-general-exact-token-rescue-v1` 각각 `searchEvaluation`
  - 결과: `PASS` — 68문항 결과 ID·상태·relevance·원래 score/distance 차이 0; P17 최종 기준과도 차이 0
- **범위:** P18 backend unit 전체
  - 명령: `.\gradlew.bat test --no-daemon --rerun-tasks`
  - 결과: `PASS` — 420개 중 405 pass, 기존 환경 조건 15 skip
- **범위:** P18.1 backend integration 전체
  - 명령: `.\gradlew.bat integrationTest --no-daemon --rerun-tasks`
  - 결과: `PASS` — stale raw-distance assertion을 repository raw order와 P4 profile final order 검증으로 분리한 뒤 78개 중 75 pass, 3 skip, 0 fail
- **범위:** P18 관련 PostgreSQL integration
  - 명령: GENERAL·completed Claim Gate·owner 격리 대상 6건
  - 결과: `PASS` — 6/6
...
아래 Run A–C는 통합 전 실행 이력으로, 모두 그 기준 위의 당시 미커밋 worktree였다.

- **실행:** Run A (`2026-08-07T08:09:50Z`)
  - source 상태: exact-token 1차 교정, 완료 행위 정규화 전
  - top-1: 7/8
  - 오타: 2/2
  - 중복: 0
  - 무관 거부: 1.0
  - 오거부: 0.125
  - 판정·raw: `FAIL`; `local/search-evaluation/prz008-tuning-v22-unicode-exact-20260807/`
- **실행:** Run B (`2026-08-07T08:21:19Z`)
  - source 상태: profile SHA-256 `100fb6b2f751b9b39334dfe4d654b7a0487fa1089634812036fb750ea4ef06c5`
  - top-1: 8/8
  - 오타: 2/2
  - 중복: 0
  - 무관 거부: 1.0
  - 오거부: 0
  - 판정·raw: `PASS`; `local/search-evaluation/prz008-tuning-v22-unicode-exact-final-20260807/`
- **실행:** Run C (`2026-08-07T09:37:12Z`)
  - source 상태: v2.3·본문 근거 전용, profile SHA-256 `e10d2d923a046855a0df1a8a81ca60b23a28c5cf20b70140ba75153a25892a83`
  - top-1: 8/8
  - 오타: 2/2
  - 중복: 0
  - 무관 거부: 1.0
  - 오거부: 0
  - 판정·raw: `PASS`; `local/search-evaluation/prz008-tuning-v23-exact-document-title-final-20260807/`
- **실행:** Run D (`2026-08-07T10:02:10Z`)
  - source 상태: 질문형 완료 표현·Unicode 복합어 경계 교정, profile SHA-256 `e72a63fe5eec96640828d402b41fb642c9fbd862049fa65b8d020a32576c79c3`
  - top-1: 8/8
  - 오타: 2/2
  - 중복: 0
  - 무관 거부: 1.0
  - 오거부: 0
  - 판정·raw: `PASS`; `local/search-evaluation/prz008-tuning-v23-p1-fix-20260807/`
- **실행:** Run E (`2026-08-07T10:51:07Z`)
  - source 상태: S2B-11 문장 양태 fail-closed, profile SHA-256 `01eb199af0ff74c427b5178cce5c312d51b85a0ec1745f87ea89804b981194c6`
  - top-1: 8/8
  - 오타: 2/2
  - 중복: 0
  - 무관 거부: 1.0
  - 오거부: 0
  - 판정·raw: `PASS`; `local/search-evaluation/prz008-tuning-v23-s2b11-20260807/`
...
통합 전 기준 commit `83631f13c21eab54ac0f32ebb0f893b6c5acea0f` 위 당시 미커밋 worktree에서
...
- **범위:** 대상 단위
  - 명령: `.\gradlew.bat test --tests "com.prizm.search.evaluation.SearchEvaluationCompositeProfileTest" --no-daemon --rerun-tasks`
  - 결과: `PASS` — 27/27, skip 0
- **범위:** 대상 PostgreSQL·pgvector
  - 명령: `.\gradlew.bat integrationTest --tests "com.prizm.infrastructure.PgVectorInfrastructureTest.optInProfileRequiresAnAssertedCompletedReleaseClaimInPostgreSql" --no-daemon --rerun-tasks`
  - 결과: `PASS` — 1/1, skip 0
- **범위:** TUNING·고정 TEST·전체 회귀
  - 명령: 실행하지 않음
  - 결과: `NOT_RUN`
...
- **범위:** 대상 PostgreSQL·pgvector
  - 명령: `.\gradlew.bat integrationTest --tests "com.prizm.infrastructure.PgVectorInfrastructureTest.optInProfileRequiresAnAssertedCompletedReleaseClaimInPostgreSql" --no-daemon --rerun-tasks`
  - 결과: `PASS` — 1/1, skip 0
- **범위:** 현재 source TUNING 15
  - 명령: `$env:PRIZM_SEARCH_EVALUATION_SPLIT='TUNING'; $env:PRIZM_SEARCH_EVALUATION_PROFILE='source-dedup-evidence-signals-v1'; .\gradlew.bat searchEvaluation --no-daemon --rerun-tasks -PsearchEvaluationDataset=src/test/resources/search-evaluation/v2-3 -PsearchEvaluationOutput=local/search-evaluation/prz008-tuning-v23-s2b11-20260807`
  - 결과: `PASS` — top-1 8/8·오타 2/2·중복 0·무관 거부 1.0·오거부 0·PDF page 1.0
- **범위:** 고정 TEST·전체 회귀·독립 감사
  - 명령: 실행하지 않음
  - 결과: `NOT_RUN`
...
- **범위:** 실패 재현
  - 명령: `.\gradlew.bat test --tests "com.prizm.search.evaluation.SearchEvaluationCompositeProfileTest" --no-daemon --rerun-tasks`
  - 결과: `FAIL` — 28개 중 신규 변환 테스트 1 fail, skip 0
- **범위:** 문장부호 경계 재현
  - 명령: 같은 명령
  - 결과: `FAIL` — 연속 마침표 변환에서 `StringIndexOutOfBoundsException` 재현
- **범위:** 대상 단위
  - 명령: 같은 명령
  - 결과: `PASS` — 28/28, skip 0
- **범위:** 대상 PostgreSQL·pgvector
  - 명령: `.\gradlew.bat integrationTest --tests "com.prizm.infrastructure.PgVectorInfrastructureTest.optInProfileRequiresAnAssertedCompletedReleaseClaimInPostgreSql" --no-daemon --rerun-tasks`
  - 결과: `PASS` — 1/1, skip 0
- **범위:** TUNING·고정 TEST·전체 회귀
...
- **범위:** 실패 재현
  - 명령: `.\gradlew.bat test --tests "com.prizm.search.evaluation.SearchEvaluationCompositeProfileTest" --no-daemon --rerun-tasks`
  - 결과: `FAIL` — 29개 중 신규 테스트 2 fail, skip 0
- **범위:** 절간 합성 자체 감사 재현
  - 명령: 같은 명령
  - 결과: `FAIL` — 29개 중 절간 합성 테스트 1 fail, skip 0
- **범위:** 대상 단위
  - 명령: 같은 명령
  - 결과: `PASS` — 29/29, skip 0
- **범위:** 대상 PostgreSQL·pgvector
  - 명령: `.\gradlew.bat integrationTest --tests "com.prizm.infrastructure.PgVectorInfrastructureTest.optInProfileRequiresAnAssertedCompletedReleaseClaimInPostgreSql" --no-daemon --rerun-tasks`
  - 결과: `PASS` — 1/1, skip 0
- **범위:** TUNING·고정 TEST·전체 회귀·OpenSQL
  - 명령: 실행하지 않음
  - 결과: `NOT_RUN`
...
- **범위:** 최초 구조 RED
  - 명령: `gradle.bat test --tests "com.prizm.search.evaluation.SearchEvaluationCompositeProfileTest" --no-daemon --rerun-tasks`
  - 결과: `FAIL` — 31개 중 4개 메서드 fail, skip 0
- **범위:** 1차 감사 finding RED
  - 명령: 같은 명령
  - 결과: `FAIL` — 31개 중 2개 메서드 fail, skip 0
- **범위:** 2차 감사 finding RED
  - 명령: 같은 명령
  - 결과: `FAIL` — 31개 중 annotation 변환 메서드 1개 fail, skip 0
- **범위:** 3차 감사 finding RED
  - 명령: 같은 명령
  - 결과: `FAIL` — 31개 중 불투명 대상 token 변환 메서드 1개 fail, skip 0
- **범위:** 4차 감사 finding RED
  - 명령: 같은 명령
  - 결과: `FAIL` — 31개 중 query 탐지·예약어 대상 메서드 2개 fail, skip 0
- **범위:** query 문법 stem 자체 감사 RED
  - 명령: 같은 명령
  - 결과: `FAIL` — 31개 중 unsupported 질의 메서드 1개 fail, skip 0
- **범위:** claim prefix 자체 감사 RED
  - 명령: 같은 명령
  - 결과: `FAIL` — 31개 중 불투명 대상 메서드 1개 fail, skip 0
- **범위:** 5차 감사 finding RED
  - 명령: 같은 명령
  - 결과: `FAIL` — 31개 중 접속형 대상 메서드 1개 fail, skip 0
- **범위:** 6차 감사 finding RED
  - 명령: 같은 명령
  - 결과: `FAIL` — 31개 중 target separator 메서드 1개 fail, skip 0
- **범위:** 내부 마침표 자체 감사 RED
  - 명령: 같은 명령
  - 결과: `FAIL` — 31개 중 unsupported 질의 메서드 1개 fail, skip 0
- **범위:** 7차 감사 finding RED
  - 명령: 같은 명령
  - 결과: `FAIL` — 31개 중 별칭 body 변환 메서드 1개 fail, skip 0
- **범위:** 최종 대상 단위
  - 명령: 같은 명령
  - 결과: `PASS` — 31/31, skip 0
- **범위:** 최종 대상 PostgreSQL·pgvector
  - 명령: `gradle.bat integrationTest --tests "com.prizm.infrastructure.PgVectorInfrastructureTest.optInProfileRequiresAnAssertedCompletedReleaseClaimInPostgreSql" --no-daemon --rerun-tasks`
  - 결과: `PASS` — 1/1, skip 0
- **범위:** 최종 독립 재감사
  - 명령: 읽기 전용 정적 감사
  - 결과: `PASS` — API 경계·양태·Dataset 보존 P0 0 / P1 0; 양태·Dataset 감사 중 테스트 재실행 없음
- **범위:** 고정 Dataset TEST 사전 실행 — legacy 기본 profile
  - 명령: `$env:PRIZM_SEARCH_EVALUATION_SPLIT='TEST'; .\gradlew.bat searchEvaluation --no-daemon --rerun-tasks -PsearchEvaluationDataset=src/test/resources/search-evaluation/v2-3 -PsearchEvaluationOutput=local/search-evaluation/prz008-test-v23-legacy-20260807`
  - 결과: `FAIL` — 1/1; 이전 runner가 `This Batch permits Dataset v2 TUNING execution only.`로 색인·검색·지표 산출 전 중단
- **범위:** v2.3 allow·fixture 선택 단위
  - 명령: `SearchEvaluationDatasetSelectorTest` 대상 단위
  - 결과: `PASS` — 4/4; v2.2·flag 누락·TUNING은 거절하고, v2.3 `TEST`·`true`와 질문 참조 fixture만 허용
- **범위:** 고정 Dataset TEST 비교 — legacy 기본 profile
  - 명령: `$env:PRIZM_SEARCH_EVALUATION_SPLIT='TEST'; $env:PRIZM_SEARCH_EVALUATION_ALLOW_FROZEN_TEST='true'; $env:PRIZM_SEARCH_EVALUATION_PROFILE='current-product'; .\gradlew.bat searchEvaluation --no-daemon --rerun-tasks -PsearchEvaluationDataset=src/test/resources/search-evaluation/v2-3 -PsearchEvaluationOutput=local/search-evaluation/prz008-test-v23-legacy-final-20260807`
  - 결과: `PASS` — 1/1; Direct MRR@5 `1.0000`, nDCG@5 `0.9710`, no-evidence rejection `0`, evidence false rejection `0`
- **범위:** 고정 Dataset TEST 비교 — opt-in profile
  - 명령: `$env:PRIZM_SEARCH_EVALUATION_SPLIT='TEST'; $env:PRIZM_SEARCH_EVALUATION_ALLOW_FROZEN_TEST='true'; $env:PRIZM_SEARCH_EVALUATION_PROFILE='source-dedup-evidence-signals-v1'; .\gradlew.bat searchEvaluation --no-daemon --rerun-tasks -PsearchEvaluationDataset=src/test/resources/search-evaluation/v2-3 -PsearchEvaluationOutput=local/search-evaluation/prz008-test-v23-opt-in-final-20260807`
  - 결과: `PASS` — 1/1; Direct MRR@5 `0.1667`, nDCG@5 `0.1667`, no-evidence rejection `1.0000`, evidence false rejection `0.8333`; 기본값 승격 보류
- **범위:** TUNING·전체 회귀·OpenSQL
  - 명령: 실행하지 않음
  - 결과: `NOT_RUN`; 직전 TUNING은 `HISTORICAL_PASS_NOT_RERUN`
...
- **지표:** Direct MRR@5
  - legacy (`current-product`): 1.0000
  - opt-in (`source-dedup-evidence-signals-v1`): 0.1667
- **지표:** nDCG@5
  - legacy (`current-product`): 0.9710
  - opt-in (`source-dedup-evidence-signals-v1`): 0.1667
- **지표:** no-evidence rejection
  - legacy (`current-product`): 0
  - opt-in (`source-dedup-evidence-signals-v1`): 1.0000
- **지표:** evidence false rejection
  - legacy (`current-product`): 0
  - opt-in (`source-dedup-evidence-signals-v1`): 0.8333
- **지표:** duplicate result ratio
  - legacy (`current-product`): 0
  - opt-in (`source-dedup-evidence-signals-v1`): 0
...
- **검증:** 변환 기반 단위 RED
  - 명령 또는 범위: `SearchEvaluationCompositeProfileTest` 대상
  - 결과: `FAIL` — 구현 전 33개 중 `supportsBoundedProjectIdentityDirectEvidenceAndExactCompletionFacts` 1개 fail; TUNING의 Lumen 직접 근거를 추가한 뒤에도 같은 대상 33개 중 1개 fail
- **검증:** 최종 대상 단위
  - 명령 또는 범위: `./gradlew.bat test --tests "com.prizm.search.evaluation.SearchEvaluationCompositeProfileTest" --no-daemon --rerun-tasks`
  - 결과: `PASS` — 33/33
- **검증:** 최종 대상 PostgreSQL·pgvector
  - 명령 또는 범위: `./gradlew.bat integrationTest --tests "com.prizm.infrastructure.PgVectorInfrastructureTest.optInProfileRequiresAnAssertedCompletedReleaseClaimInPostgreSql" --no-daemon --rerun-tasks`
  - 결과: `PASS` — 1/1
- **검증:** 최초 TUNING
  - 명령 또는 범위: v2.3 `TUNING`, opt-in
  - 결과: `FAIL` — runner 1/1이나 Direct MRR@5/@20 `0.8750`, 근거 오거부 `0.1250`; Lumen 직접 근거 문법 누락을 재현
- **검증:** 최종 TUNING
  - 명령 또는 범위: v2.3 `TUNING`, opt-in, 15문항
  - 결과: `PASS` — Direct MRR@5/@20 `1.0000`, nDCG@5 `0.9783`, 중복 `0`, 무근거 거부 `1.0`, 근거 오거부 `0`, total p95 `138ms`
- **검증:** 고정 TEST — legacy
  - 명령 또는 범위: v2.3 `TEST`, allow flag, `current-product`
  - 결과: `PASS` — 1/1; Direct MRR@5/@20 `1.0000`, nDCG@5 `0.9710`, total p95 `138ms`
- **검증:** 고정 TEST — opt-in
  - 명령 또는 범위: v2.3 `TEST`, allow flag, opt-in
  - 결과: `PASS` — 1/1; Direct MRR@5/@20 `1.0000`, nDCG@5 `0.9710`, top-1 직접 근거 `1.0`, 무근거 거부 `1.0`, 근거 오거부 `0`, 중복 `0`, PDF page `1.0`, total p95 `160ms` (+`15.9%`)
...
다음 기록은 S2C-02 직후 아직 OpenSQL·전체 회귀를 실행하지 않았던 시점의 이력이다.
이후 OpenSQL 제품 Gate, 기본 profile 승격과 전체 회귀를 완료했으며 최종 결과는 이
문서 상단 판정과 아래 후속 실행 기록을 따른다.
...
당시 남은 Gate 전에는 새 profile을 기본값으로 승격하거나 PRZ-008을 `VERIFIED`로
표시하지 않는 조건이었다.
...
- **검증:** Docker·Compose
  - 명령 또는 범위: `docker version`, `docker compose config --quiet`
  - 결과: `PASS` — Engine 29.6.2 Linux, Compose config 통과
- **검증:** 초기 전체 integration
  - 명령 또는 범위: `./gradlew.bat integrationTest --no-daemon --rerun-tasks`
  - 결과: `FAIL` — 78건 중 2 fail·3 skip; stale dense fixture 기대와 기본 profile 결과 불일치
- **검증:** 대상 재현
  - 명령 또는 범위: 두 실패 메서드 대상 integration
  - 결과: `FAIL` — 2/2 재현
- **검증:** 교정 대상 integration
  - 명령 또는 범위: 같은 두 메서드
  - 결과: `PASS` — 2/2
- **검증:** backend integration 전체
  - 명령 또는 범위: `./gradlew.bat integrationTest --no-daemon --rerun-tasks`
  - 결과: `PASS` — 78건 중 75 pass·3 skip·실패 0
- **검증:** backend unit 전체
  - 명령 또는 범위: `./gradlew.bat test --no-daemon --rerun-tasks`
  - 결과: `PASS` — 350건 중 335 pass·15 skip·실패 0
...
- **검증:** snippet·SearchService focused unit
  - 결과: `PASS` — 26/26; Spring Boot 경험 문맥, 인증 문제·해결 우선, cross-document exact-content 축약
- **검증:** P8 40
  - 결과: `PASS` — Top1 `28/34`, Recall@5 `29/34`, 무근거 오탐 `1/6`
- **검증:** P17 28
  - 결과: `PASS` — Top1 `9/15`, Recall@5 `10/15`, 무근거 오탐 `1/13`
- **검증:** P18 기준 JSON 비교
  - 결과: `PASS` — 68문항 결과 ID·순서·상태·relevance 차이 `0`
- **검증:** backend unit 전체
  - 결과: `PASS` — 423개 중 408 pass·15 skip·실패 0
- **검증:** backend integration 전체
  - 결과: `PASS` — 78개 중 75 pass·3 skip·실패 0
