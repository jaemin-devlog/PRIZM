...
같은 `bge-m3` 모델로 아래 세 문장을 임베딩해 저장했다.

1. 연차 신청은 인사 시스템에서 진행합니다.
2. 서버 장애가 발생하면 운영 담당자에게 보고합니다.
3. 기밀 문서는 외부 AI 서비스에 전송할 수 없습니다.

질문 `휴가는 어디에서 신청하나요?`를 `POST /api/search`로 호출한 결과는 다음과 같다.

| 순위 | 문장 | distance | score |
|---:|---|---:|---:|
| 1 | 연차 신청은 인사 시스템에서 진행합니다. | 0.03939452 | 0.96060548 |
| 2 | 서버 장애가 발생하면 운영 담당자에게 보고합니다. | 0.04048957 | 0.95951043 |
| 3 | 기밀 문서는 외부 AI 서비스에 전송할 수 없습니다. | 0.14052298 | 0.85947702 |

REST API는 1위 문장과 동일한 `distance`, `score`를 반환했다.
