...
| PRZ-017 | 채용공고 deterministic segmentation → modal 선택 → 기존 Search → 전용 requirement/document/Evidence workspace와 PDF/TXT 이동 V1을 source `de98bcf`에 구현. 새 UI 자동 검증·독립 감사 `PASS`, 인증 브라우저 Gate `BLOCKED_BY_AUTH_ENVIRONMENT`; 전체 44개 원문/TXT 재검증은 `NOT_RUN` (`IN_PROGRESS`) |
...
- 현재 상태 `IN_PROGRESS`, source `de98bcf`. 새 UI 기준 focused backend
