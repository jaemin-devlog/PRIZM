...
public final class StructuralRetrievalPassageBuilder {
    ...
    private boolean sameAdjacentParent(EvidenceChild previous, EvidenceChild next) {
        ...
        return left.documentId() == right.documentId()
                ...
                && left.sourceUnitKey().equals(right.sourceUnitKey())
                && left.sourcePath().equals(right.sourcePath())
                && Objects.equals(left.pageNo(), right.pageNo())
    ...
    private RetrievalPassage toPassage(List<EvidenceChild> children, int passageIndex) {
        ...
        return new RetrievalPassage(
                provenance.sourceUnitKey() + "-RP-%04d".formatted(passageIndex),
                provenance.documentId(),
                provenance.documentVersionId(),
                provenance.sourceUnitKey(),
            ...
            if (previous != null) {
                ...
                if (left.documentId() == right.documentId()
                    ...
                    throw new IllegalArgumentException(
                            "EvidenceChild input must preserve non-overlapping source order");
                }
            }
            previous = child;
        }
    ...
    private void validateEveryChildAppearsOnce(
        ...
        if (!expected.equals(actual)) {
            throw new IllegalStateException(
                    "RetrievalPassage construction lost or reordered EvidenceChild IDs");
        }
    }
