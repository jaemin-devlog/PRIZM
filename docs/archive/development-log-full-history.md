...
> **보관 안내:** 이 문서는 작성 당시의 전체 개발 기록입니다. 과거 계획, 실험,
> 실패와 판정을 원문으로 보존하며 현재 제품 범위나 현재 backlog로 해석하지
> 않습니다. 현재 구현은 [현재 구현 현황](../project-status.md), 현재 Spec 상태는
> [Spec Registry](../../specs/README.md)를 기준으로 확인합니다.
...
- 계획: 구현 기준선, 거버넌스, Quickstart, port/adapter 경계, canonical source, CareerFact, portfolio, API v1, 멀티모듈 패키징, 기관 scope, 최종 감사의 11단계 실행 계획을 당시 보관 문서에 기록했다. 해당 보관 문서는 현재 저장소에서 제거됐으며, 현재 개발 순서는 [개발 로드맵](../roadmap.md)을 따른다.
